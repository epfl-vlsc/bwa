...
int mem_sam_pe_results(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const mem_pestat_t pes[4], uint64_t id, bseq1_t s[2], mem_alnreg_v a[2], 
    mem_aln_t aa[2][2], int n_aa[2])
{
	extern int mem_mark_primary_se(const mem_opt_t *opt, int n, mem_alnreg_t *a, int64_t id);
	extern int mem_approx_mapq_se(const mem_opt_t *opt, const mem_alnreg_t *a);
	extern void mem_reg2result(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, bseq1_t *s, mem_alnreg_v *a, int extra_flag, 
      const mem_aln_t *m, mem_aln_t alignments[2], int* n_aa);
	extern char **mem_gen_alt(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const mem_alnreg_v *a, int l_query, const char *query);
...
no_pairing:
  printf("got to no pairing\n");
	...
	mem_reg2result(opt, bns, pac, &s[0], &a[0], 0x41|extra_flag, &h[1], aa[0], &n_aa[0]);
	mem_reg2result(opt, bns, pac, &s[1], &a[1], 0x81|extra_flag, &h[0], aa[1], &n_aa[1]);
	if (strcmp(s[0].name, s[1].name) != 0) err_fatal(__func__, "paired reads have different names: \"%s\", \"%s\"\n", s[0].name, s[1].name);
	//free(h[0].cigar); free(h[1].cigar);
	return n;
}
